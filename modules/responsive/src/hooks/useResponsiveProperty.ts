...
const useResponsiveProperty = (selection?: string[]) => {
  ...
  return <T>(property: ResponsiveProperty<T>) => {
    ...
    if (Array.isArray(property)) {
      ...
      validValues = validSelection.reduce<ResponsivePropertyObject<T>>((res, key, index) => {
    ...
    const key = [...validSelection].reverse().find((k: string) => {
      const value = validValues[k]
      return !!value
    })
...
export default useResponsiveProperty
